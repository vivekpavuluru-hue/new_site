    ...
    <div class="container">
        <div class="d-flex justify-between align-center mt-2rem mb-2rem">
            ...
            <div class="d-flex gap-15 align-center">
                <form action="index.php" method="GET" class="filter-form">
                    <input type="hidden" name="route" value="openPos">
                    <div>
            ...
            <table class="data-table-centered">
                <tr>
                    <?php foreach ($tableData['columns'] as $colName): ?>
                        <?php if ($colName !== '_po_id'): ?>
                            <th><?php echo htmlspecialchars(ucwords(str_replace('_', ' ', $colName))); ?></th>
                        <?php endif; ?>
                    <?php endforeach; ?>
                </tr>
                <?php foreach ($tableData['data'] as $row): ?>
                    <tr class="clickable-row" onclick="window.location='index.php?route=poDetails&po_id=<?php echo urlencode($row['_po_id']); ?>'">
                        <?php foreach ($row as $colName => $data): ?>
                            <?php 
                            // Skip displaying hidden columns
                            if ($colName === '_po_id') continue;
        ...
        <?php if ($totalPages > 1): ?>
            <div class="pagination">
                <?php if ($page > 1): ?>
                    <a href="?route=openPos&limit=<?php echo $recordsPerPage; ?>&page=<?php echo $page - 1; ?>" class="page-link">&laquo; Previous</a>
                <?php endif; ?>
                
                <span class="page-info">Page <?php echo $page; ?> of <?php echo $totalPages; ?></span>
                
                <?php if ($page < $totalPages): ?>
                    <a href="?route=openPos&limit=<?php echo $recordsPerPage; ?>&page=<?php echo $page + 1; ?>" class="page-link">Next &raquo;</a>
                <?php endif; ?>
            </div>
        <?php endif; ?>
